...
export default function DayHabitTableLayout({ children, date }: DayHabitTableLayoutProps) {
    ...
    return (
        ...
        >
            <Accordion className="flex flex-col" expanded={isExpanded} onChange={() => setIsExpanded(!isExpanded)}>
                <AccordionSummary expandIcon={<ExpandMoreIcon color={`${today ? 'secondary' : 'primary'}`} />}>
